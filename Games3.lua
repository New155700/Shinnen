...
-- 1. ดึง Library จากลิงก์ Gist ของคุณ (ห้ามเปลี่ยน URL นี้)
...
-- 1. สร้างหน้าต่าง (Win) และหน้าเมนู (Main)
local Win = NScanner:CreateWindow()
local Main = Win:CreateTab("Main")

-- 2. เชื่อมปุ่มเข้ากับตัวแปรในวงจรโกงของคุณ
-- (เปลี่ยนชื่อ getgenv().Speed หรือ getgenv().AutoE ให้ตรงกับที่คุณเขียนไว้ในวงจร)

Main:CreateInput({
    Name = "⚡ WalkSpeed",
    Callback = function(v)
        getgenv().Speed = tonumber(v) or 16 -- ส่งค่าไปที่วงจรวิ่งไวของคุณ
    end
})
...
Main:CreateToggle({
    Name = "⌨️ Auto Press E",
    Callback = function(v)
        getgenv().AutoE = v -- ส่งค่าไปที่วงจรกด E ของคุณ
    end
})
